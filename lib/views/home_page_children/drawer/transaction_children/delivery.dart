...
class DeliveryTransaction extends StatefulWidget {
  @override
  _DeliveryTransactionState createState() => _DeliveryTransactionState();
}

class _DeliveryTransactionState extends State<DeliveryTransaction> {
  double total = 0.0;
  @override
  void initState() {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: StreamBuilder<List>(
        ...
        builder: (_, result){
          try{
            List data = result.data.where((element) => int.parse(element['isDelivery'].toString()) == 1).toList();
            if(data.length > 0 ) {
            ...
            return Center(
              child: Text("You have no order to be delivered",style: TextStyle(
